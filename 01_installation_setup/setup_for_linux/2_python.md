# Installing Python with uv (Linux / Ubuntu)

## Goal

Install Python using uv. You do **not** need `apt install python3`, a
python.org download, or pyenv — uv downloads and manages its own standalone
Python builds, and can even fetch Python automatically the first time a
project needs it.

## Prerequisites

- uv installed → see [1_uv.md](1_uv.md)
...
- **Wrong Python is used** → uv prefers its own managed Pythons, but falls
  back to system Python. Force managed Python with `uv run --managed-python ...`
  or check `uv python list`.
- **Old pyenv shims interfere** → if `which python` still points into
  `~/.pyenv`, remove the pyenv lines from `~/.bashrc` / `~/.zshrc` and restart
  the terminal. uv does not need pyenv.
